...
static void dbg_ptr(const char* tag, const void* p){
  ...
  if (cudaPointerGetAttributes(&attr, p) == cudaSuccess) {
    if (attr.type == cudaMemoryTypeDevice) {
      std::cerr<<"[ep_mpi_debug] "<<tag<<" ptr="<<p<<" type=Device devId="<<attr.device<<"\n";
    } else if (attr.type == cudaMemoryTypeHost) {
      std::cerr<<"[ep_mpi_debug] "<<tag<<" ptr="<<p<<" type=Host\n";
    } else {
      std::cerr<<"[ep_mpi_debug] "<<tag<<" ptr="<<p<<" type=Unknown\n";
    }
  } else {
    std::cerr<<"[ep_mpi_debug] "<<tag<<" ptr="<<p<<" type=? (attr query failed)\n";
  }
#else
  std::cerr<<"[ep_mpi_debug] "<<tag<<" ptr="<<p<<"\n";
#endif
*/
}
